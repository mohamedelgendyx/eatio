        ...
        for i in intractableObjects:
              if i.rotY == 90 or  i.rotY == -90:
                      if test_cu(player.radius, player.posX, player.posZ, i.width , i.Length, i.posX,i.posZ) == True:
                            if i.Length * i.width > pi* player.radius * player.radius:
                                  i.a = 0.4
                            else :
                                  i.posY-=0.2
                      else:
...
def onSpecialKeyDown(key, x, y):
    ...
    if key == GLUT_KEY_RIGHT:
        if player.posX < 100-player.radius:
            player.startAnimation(DeltaAnimation(AnimationParams.posX,+PLAYER_MINIMUM_MOVEMENT ,.01))
        player.rotY = 90
        ...
        if player.posZ < -player.radius:
            player.startAnimation(DeltaAnimation(AnimationParams.posZ, +PLAYER_MINIMUM_MOVEMENT , .01))
        player.rotY = 0
